...
typedef _FheEncryptC = Int32 Function(Pointer<Uint8>, Size, Pointer<Int64>,
    Size, Uint32, Uint32, Pointer<Pointer<Uint8>>, Pointer<Size>);
typedef _FheEncryptDart = int Function(Pointer<Uint8>, int, Pointer<Int64>, int,
    int, int, Pointer<Pointer<Uint8>>, Pointer<Size>);
...
typedef _FheDecryptC = Int32 Function(Pointer<Uint8>, Size, Pointer<Uint8>,
    Size, Uint32, Uint32, Pointer<Pointer<Int64>>, Pointer<Size>);
typedef _FheDecryptDart = int Function(Pointer<Uint8>, int, Pointer<Uint8>, int,
    int, int, Pointer<Pointer<Int64>>, Pointer<Size>);
...
//     uint8_t **ct_out, size_t *ct_len,
//     uint32_t *n_cts_out)
typedef _FheDeserializeValueC = Int32 Function(Pointer<Uint8>, Size,
    Pointer<Pointer<Uint8>>, Pointer<Size>, Pointer<Uint32>);
typedef _FheDeserializeValueDart = int Function(Pointer<Uint8>, int,
    Pointer<Pointer<Uint8>>, Pointer<Size>, Pointer<Uint32>);
...
class FheNative {
  ...
  FheNative() {
    ...
    _freeI64Buf =
        lib.lookupFunction<_FheFreeI64C, _FheFreeI64Dart>('fhe_free_i64_buf');
    _lweEncryptSeeded =
        lib.lookupFunction<_FheLweEncryptSeededC, _FheLweEncryptSeededDart>(
    ...
    _serializeValue =
        lib.lookupFunction<_FheSerializeValueC, _FheSerializeValueDart>(
            'fhe_serialize_value');
    _deserializeValue =
        lib.lookupFunction<_FheDeserializeValueC, _FheDeserializeValueDart>(
  ...
  KeygenResult keygen(Uint64List topology) {
    ...
    try {
      final rc =
          _keygen(topoPtr, topology.length, ckPtrPtr, ckLen, skPtrPtr, skLen);
      ...
      malloc.free(topoPtr);
      malloc.free(ckPtrPtr);
      malloc.free(ckLen);
      malloc.free(skPtrPtr);
      malloc.free(skLen);
    }
  }
  ...
  Uint8List encrypt(
      Uint8List clientKey, Int64List values, int bitWidth, bool isSigned) {
    ...
    try {
      final rc = _encrypt(ckPtr, clientKey.length, valPtr, values.length,
          bitWidth, isSigned ? 1 : 0, ctPtrPtr, ctLen);
      if (rc != 0) throw StateError('fhe_encrypt failed (code $rc)');
      return _readAndFree(ctPtrPtr.value, ctLen.value);
    } finally {
      malloc.free(ckPtr);
      malloc.free(valPtr);
      malloc.free(ctPtrPtr);
      malloc.free(ctLen);
    }
  }
  ...
  Int64List decrypt(
      Uint8List clientKey, Uint8List ciphertext, int bitWidth, bool isSigned) {
    final ckPtr = _toNativeUint8(clientKey);
    ...
    try {
      final rc = _decrypt(ckPtr, clientKey.length, ctPtr, ciphertext.length,
          bitWidth, isSigned ? 1 : 0, outPtrPtr, outLen);
  ...
  Int64List lweDecryptFull(Uint8List clientKey, Uint8List ciphertext, int nCts,
      int encodingWidth, bool isSigned, int lweDimension) {
    final ckPtr = _toNativeUint8(clientKey);
    final ctPtr = _toNativeUint8(ciphertext);
    final outPtrPtr = malloc<Pointer<Int64>>();
    final outLen = malloc<Size>();
    try {
      final rc = _lweDecryptFull(
          ckPtr,
          clientKey.length,
          ctPtr,
          ciphertext.length,
  ...
  Uint8List serializeValue(
      Uint8List ctData,
      List<int> shape,
      List<int> abstractShape,
      int encodingWidth,
      bool isSigned,
      int lweDimension,
      int keyId,
    ...
    final outLen = malloc<Size>();
    try {
      final rc = _serializeValue(
          ctPtr,
          ctData.length,
          shapePtr,
          shape.length,
          absShapePtr,
          abstractShape.length,
          encodingWidth,
          isSigned ? 1 : 0,
          lweDimension,
          keyId,
  ...
  (Uint8List, int) deserializeValue(Uint8List data) {
    ...
    try {
      final rc =
          _deserializeValue(dataPtr, data.length, ctPtrPtr, ctLen, nCtsPtr);
